...
describe("parseMacrosRaw", () => {
  ...
  it("should ignore keys which don't have a base 10 code", () => {
    expect(parseMacrosRaw("8 F 8 22 0 0", [])).toEqual([
      { id: 0, name: "", actions: [{ keyCode: 22, type: 8 }], macro: ""},
    ]);
  });

  it("should ignore keys which don't have a base 10 type", () => {
    expect(parseMacrosRaw("F 22 8 22 0 0", [])).toEqual([
      { id: 0, name: "", actions: [{ keyCode: 22, type: 8 }], macro: ""},
    ]);
  });
  ...
  it("GAP: Type 1 never makes it into the actions list", () => {
    expect(parseMacrosRaw("1 1 2 3 4 0 0", [])).toEqual([
      { id: 0, name: "", actions: [], macro: ""},
    ]);
  });
  ...
  test.each([
    ...
    {type: 4},
    {type: 5},
  ])("should parse key of type $type using the next 2 bytes", ({type}) => {
    //    1 = 0b0000_0001
    // << 8 = 0b0000_0001_0000_0000
    //  + 2 = 0b0000_0001_0000_0010
    expect(parseMacrosRaw(`${type} 1 2 0 0`, [])).toEqual([
      { id: 0, name: "", actions: [{ keyCode: 0b0000_0001_0000_0010, type: type }], macro: ""},
    ]);
  });
  ...
  test.each([
    ...
    {type: 7},
    {type: 8},
  ])("should parse key of type $type using the next 1 bytes", ({type}) => {
    expect(parseMacrosRaw(`${type} 1 0 0`, [])).toEqual([
      { id: 0, name: "", actions: [{ keyCode: 1, type: type }], macro: ""},
    ]);
  });
...
describe("serializeMacrosRaw", () => {
  ...
  it("should serialize a macro with a type 1", () => {
    const macros: MacrosType[] = [
      { id: 0, name: "", actions: [
        { keyCode: [0b0000_0001_0000_0010, 0b0001_0000_0010_0000], type: 1 },
      ], macro: ""},
    ];
  ...
  it("GAP: serializing type 1 key code more than 16 bits results in a first number larger than 8-bit", () => {
    const macros: MacrosType[] = [
      { id: 0, name: "", actions: [
          { keyCode: [0b0001_0000_0001_0000_0010, 0b0001_0001_0000_0010_0000], type: 1 },
        ], macro: ""},
    ];
  ...
  test.each([
    ...
    {type: 4},
    {type: 5},
  ])("should serialize macro with type $type", ({type}) => {
    const macros: MacrosType[] = [
      { id: 0, name: "", actions: [
        { keyCode: 0b0000_0001_0000_0010, type: type },
      ], macro: ""},
    ];
  ...
  it("GAP: serializing type 2-5 key code more than 16 bits results in a first number larger than 8-bit", () => {
    const macros: MacrosType[] = [
      { id: 0, name: "", actions: [
          { keyCode: 0b0001_0000_0001_0000_0010, type: 2 },
        ], macro: ""},
    ];
