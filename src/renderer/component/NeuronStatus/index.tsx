...
const ConnectedImage = (): JSX.Element => {
...
interface DisconnectedDeviceProps {
  isLoading: boolean;
}
const DisconnectedDevice = ({ isLoading }: DisconnectedDeviceProps) => {
  ...
  return (
    <g className="connectionLoading connectionScanning ">
      ...
      <defs>
        ...
        <image
          className={`rotating ${isLoading ? "loading" : ""}`}
    ...
    <Style>
      ...
      >
        ...
        <svg width={157} height={179} fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          />
          {deviceItems && !isDeviceConnected ? <DisconnectedDevice isLoading={!!loading || scanFoundDevices} /> : ""}

          {!deviceItems && !isDeviceConnected ? <NoDevices /> : ""}

          {isDeviceConnected ? <ConnectedImage /> : ""}
        </svg>
      </div>
    </Style>
  );
