...
interface TitleProps {
  ...
  variant?: "danger" | "error" | "success" | "warning";
  svgICO?: React.ReactNode | null;
  children: React.ReactNode;
}
function Heading({ headingLevel, size, className, color, children, variant, svgICO = null }: TitleProps) {
  const Tag = `h${headingLevel}` as keyof JSX.IntrinsicElements;
  return (
    <HeadingWrapper>
      <Tag
        className={`${size || ""} ${variant && "titleAlert"} ${variant || ""} ${color || ""} ${svgICO ? "hasIcon" : ""} ${
          className || ""
        }`}
      >
        {svgICO}
